...
- 安全审核：支持提示词屏蔽词、AI 提示词审核、AI 图片审核、图片审核未通过处理和审核白名单。
...
| 生成结果发送方式     | 普通图片消息         | `/生图` 指令完成后的自动发送方式；可选普通图片消息或合并转发消息。合并转发仅在 aiocqhttp / OneBot v11 平台尝试发送；其他平台直接使用普通图片消息，发送失败时也会自动回退。 |
...
| 图片审核未通过处理方式     | 可选不发送或合并转发消息；合并转发仅支持 aiocqhttp / OneBot v11，且只复用现有图片审核未通过结果，不新增额外检测。 |
...
- 生成结果发送方式中的“合并转发消息”仅覆盖 `/生图` 指令结果；LLM 工具任务仍由 AI 通过 `send_message_to_user` 交付结果。该模式仅在 aiocqhttp / OneBot v11 平台尝试发送，其他平台直接使用普通图片消息。
- 图片审核未通过处理默认不发送；选择“合并转发消息”后，仅在 aiocqhttp / OneBot v11 平台按聊天记录形式发送审核未通过的生成结果，非 aiocqhttp、LLM 工具交付、公共 API 或合并转发失败时仍按不发送处理。
